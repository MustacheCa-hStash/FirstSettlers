Shader "Custom/OakBarkSimpleLit"
{
    Properties
    {
        [MainTexture] _BaseMap("Oak Bark Texture", 2D) = "white" {}
        [MainColor] _BaseColor("Base Bark Color", Color) = (0.34, 0.25, 0.14, 1.0)
        _RidgeColor("Warm Ridge Color", Color) = (0.54, 0.41, 0.24, 1.0)
        _CreviceColor("Dark Crevice Color", Color) = (0.035, 0.025, 0.018, 1.0)
        _CoolShadowColor("Cool Side Color", Color) = (0.24, 0.22, 0.18, 1.0)
        _TreeBarkTint("Per Tree Bark Tint", Color) = (1, 1, 1, 1)
        _GrooveThreshold("Groove Threshold", Range(0, 1)) = 0.46
        _GrooveSoftness("Groove Softness", Range(0.001, 0.35)) = 0.09
        _GrooveStrength("Groove Strength", Range(0, 1)) = 0.92
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                half verticalNoise = ValueNoise(IN.uv * float2(2.0, 9.5));
                half fineNoise = ValueNoise(IN.uv * float2(14.0, 31.0));
                half worldNoise = ValueNoise(IN.positionWS.xz * 0.34 + IN.uv.yx * 1.7);
                half variation = verticalNoise * 0.45h + fineNoise * 0.25h + worldNoise * 0.30h;
