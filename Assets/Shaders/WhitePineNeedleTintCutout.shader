Shader "Custom/WhitePineNeedleSimpleLitCutout"
{
    Properties
    {
        ...
        _WindSpeed("Wind Canopy Sway Speed", Range(0, 8)) = 1.2
        _WindFlutterStrength("Wind Needle Flutter Strength", Range(0, 1)) = 0.03
        _WindFlutterSpeed("Wind Needle Flutter Speed", Range(0, 16)) = 5.8
        _WindGustScale("Wind Gust Scale", Range(0.01, 2)) = 0.2
        _WindHeightMin("Wind Height Min", Float) = 0
        _WindHeightMax("Wind Height Max", Float) = 9
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 ApplyLeafWind(float3 positionWS, float3 positionOS, float2 uv)
            {
                ...
                float cardPhase = Hash12Float(floor(positionWS.xz * 0.85) + floor(uv * 9.0)) * 6.2831853;
                float flutter = sin(_Time.y * _WindFlutterSpeed + cardPhase + spatialPhase * 2.2);
                ...
                offset.y = flutter * _WindFlutterStrength * 0.16 * heightMask;
