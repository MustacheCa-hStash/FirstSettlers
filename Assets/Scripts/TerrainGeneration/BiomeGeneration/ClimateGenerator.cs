...
{
    public static int GetClimateOctaveCount(int octaves)
    {
        return System.Math.Max(1, octaves - 2);
    }

    public static float GetMaxPossibleNoise(int octaves, float persistence)
    {
    ...
    public static NativeArray<float2> CreateOctaveOffsets(int seed, int octaves, Allocator allocator)
    {
        NativeArray<float2> octaveOffsets =
            new NativeArray<float2>(octaves, allocator, NativeArrayOptions.UninitializedMemory);
    ...
    public static float SampleClimate01(
        float worldX,
        float worldZ,
        int seed,
        float sampleScale,
        ...
        NativeArray<float2> octaveOffsets)
    {
        ...
        return Normalize01(noise, maxPossibleNoise);
    }
        ...
        float lacunarity, ChunkCoord chunkCoord)
    {
        ...
        float maxPossibleNoise = GetMaxPossibleNoise(noiseOctaves, noisePersistence);
